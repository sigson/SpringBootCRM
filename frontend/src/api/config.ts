...
// Публікуємо підсумок назад у window.__API_BASE__.
//
// Модулі (src/modules/sqlworkbench, src/modules/dcs) навмисно не імпортують код
// host-додатка — вони читають цей глобал. Поки index.html виставляв його
// беззастережно, вони бачили те саме, що й host. Коли значення переїхало у
// VITE_API_BASE (щоб деплой збирався без правки статики), глобал лишився
// порожнім, модулі впали у same-origin і пішли запитами на власний домен
// фронтенду: у SQL Workbench зникали з'єднання, у конструкторі звітів —
// набори даних.
//
// Тому єдине джерело правди тут, а глобал — його публікація. Якщо його вже
// виставили ззовні (escape hatch у dist/index.html), присвоєння нічого не
// змінює: resolved у такому разі з нього ж і взявся.
